...
class Paths
{
	...
	inline public static var SOUND_EXT_ALT = "wav";
	...
	static function songFile(song:String, file:String, isInst:Bool = false):Any // returns the path to use
	{
		#if MODS_ALLOWED
		var file:Sound = returnSongFile(modsSongs(song.toLowerCase().replace(' ', '-') + '/$file'));
		if (file != null)
		{
			return file;
		}
		#end
		var toReturn:String = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/$file.$SOUND_EXT';
		if (FileSystem.exists('assets/songs/${song.toLowerCase().replace(' ', '-')}/$file.$SOUND_EXT_ALT'))
		{
			toReturn = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/$file.$SOUND_EXT_ALT';
		}
		if (isInst)
		{
			if (ClientPrefs.oldSongs && FileSystem.exists('assets/songs/${song.toLowerCase().replace(' ', '-')}/OldInst.$SOUND_EXT'))
			{
				toReturn = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/OldInst.$SOUND_EXT';
				if (FileSystem.exists('assets/songs/${song.toLowerCase().replace(' ', '-')}/OldInst.$SOUND_EXT_ALT'))
				{
					toReturn = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/OldInst.$SOUND_EXT_ALT';
				}
			}
			else
			{
				toReturn = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/Inst.$SOUND_EXT';
				if (FileSystem.exists('assets/songs/${song.toLowerCase().replace(' ', '-')}/Inst.$SOUND_EXT_ALT'))
				{
					toReturn = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/Inst.$SOUND_EXT_ALT';
				}
			}
		}
		return toReturn;
	...
	inline static public function voices(song:String, file:String = 'Voices'):Any
	{
		#if MODS_ALLOWED
		...
		#end
		var toReturn:String = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/Voices.$SOUND_EXT';
		if (FileSystem.exists('assets/songs/${song.toLowerCase().replace(' ', '-')}/Voices.$SOUND_EXT_ALT'))
		{
			toReturn = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/Voices.$SOUND_EXT_ALT';
		}
		return toReturn;
	}

	inline static public function inst(song:String, file:String = 'Inst'):Any
	{
		return songFile(song, file, true);
	}

	/*inline static public function oldInst(song:String):Any
		{
			#if MODS_ALLOWED
			var file:Sound = returnSongFile(modsSongs(song.toLowerCase().replace(' ', '-') + '/OldInst'));
			if (file != null)
			{
				return file;
			}
			#end
			var toReturn:String = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/OldInst.$SOUND_EXT';
			if (FileSystem.exists('assets/songs/${song.toLowerCase().replace(' ', '-')}/OldInst.wav'))
			{
				toReturn = 'songs:assets/songs/${song.toLowerCase().replace(' ', '-')}/OldInst.wav';
			}
			return toReturn;
	}*/
	inline static public function erectVoices(song:String):Any
	{
		return voices(song, 'ErectVoices');
	}

	inline static public function erectInst(song:String):Any
	{
		return songFile(song, 'ErectInst', false);
	}
