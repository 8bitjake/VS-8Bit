...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		var path = "skins/" + Paths.currentModDirectory + "/images/gfDanceTitle.json";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)) {
			path = "skins/images/gfDanceTitle.json";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)) {
		...
		#if (polymod && !html5)
		if (sys.FileSystem.exists('skins/')) {
			var folders:Array<String> = [];
			for (file in sys.FileSystem.readDirectory('skins/')) {
				var path = haxe.io.Path.join(['skins/', file]);
				if (sys.FileSystem.isDirectory(path)) {
					folders.push(file);
				}
			}
			if(folders.length > 0) {
				polymod.Polymod.init({modRoot: "skins", dirs: folders});
			}
		}
		#end
		...
		FlxG.save.bind('vs_8bit', '8bitjake');
	...
	function startIntro()
	{
		...
		#if (desktop && MODS_ALLOWED)
		var path = "skins/" + Paths.currentModDirectory + "/images/logoBumpin.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "skins/images/logoBumpin.png";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
		...
		#if (desktop && MODS_ALLOWED)
		var path = "skins/" + Paths.currentModDirectory + "/images/gfDanceTitle.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "skins/images/gfDanceTitle.png";
		//trace(path, FileSystem.exists(path));
		}
		if (!FileSystem.exists(path)){
		...
		#if (desktop && MODS_ALLOWED)
		var path = "skins/" + Paths.currentModDirectory + "/images/titleEnter.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "skins/images/titleEnter.png";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('kav_pfp'));
		add(ngSpr);
		ngSpr.visible = false;
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 9:
					...
					if(curWacky[2] != null){
						nextWacky = curWacky[2];
					} else {
						nextWacky = curWacky[1];
					}
				// credTextShit.visible = true;
				case 10:
					if(nextWacky == curWacky[2]) addMoreText(curWacky[1]);
				case 11:
					addMoreText(nextWacky);
				// credTextShit.text += '\nlmao';
				case 12:
					deleteCoolText();
				// credTextShit.visible = false;
