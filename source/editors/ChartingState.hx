...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	public static var isErect = false;
	...
	var UI_songDifficulty:FlxUIInputText;
	...
	function addSongUI():Void
	{
		...
		UI_songDifficulty = new FlxUIInputText(saveEvents.x, loadAutosaveBtn.y, 70, '', 8);
		blockPressWhileTypingOn.push(UI_songDifficulty);
	...
	function loadSong():Void
	{
		...
		var path = Paths.voices(currentSongName);
		if (isErect) {
			path=Paths.erectVoices(currentSongName);
		}
		var file:Dynamic = path;
		vocals = new FlxSound();
	...
	function generateSong() {
		var path = Paths.inst(currentSongName);
		if (isErect) {
			path=Paths.erectInst(currentSongName);
		}
		FlxG.sound.playMusic(path, 0.6/*, false*/);
	...
	{
		var suffix = '-' + UI_songDifficulty.text;
			if (UI_songDifficulty.text == '' || UI_songDifficulty.text == 'normal') {
				suffix = '';
			}
		if (UI_songDifficulty.text  == 'erect') isErect = true;
		else isErect = false;
		trace('ERECT DEEZ NUTS: ' + isErect);
		PlayState.SONG = Song.loadFromJson(song.toLowerCase() + suffix, song.toLowerCase());
		PlayState.isErect = isErect;
		MusicBeatState.resetState();
	}
