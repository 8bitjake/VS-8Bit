...
class MainMenuState extends MusicBeatState
{
	...
	public static var vs8bitVersion:String = '3.0.0';
	...
	var easterEggKeys:Array<String> = [
		'MENTAL', 'DEATH', 'COPE', 'ACHIEVEMENT', 'DAKOTA', '2BIT'
	];
	...
	override function update(elapsed:Float)
	{
		...
		menuItems.forEach(function(spr:FlxSprite){
			if(spr.ID != curSelected) spr.x = 100;
		});
				...
				if(allowedKeys.contains(keyName)) {
					...
					for (wordRaw in easterEggKeys)
					{
						...
						if (easterEggKeysBuffer.contains(word))
						{
							...
							PlayState.SONG = Song.loadFromJson(word.toLowerCase()+'-hard', Paths.formatToSongPath(word.toLowerCase()));
