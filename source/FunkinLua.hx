...
Lua_helper.add_callback(lua, "loadGraphic", function(obj:String, img:String)
{
	if (PlayState.instance.modchartSprites.exists(obj))
	{
		PlayState.instance.modchartSprites.get(obj).loadGraphic(Paths.image(img));
		return;
	}
	...
	if (object != null)
	{
		object.loadGraphic(Paths.image(img));
	}
});
...
Lua_helper.add_callback(lua, "getDialogueFromText", function(prefix:String = '')
{
	var text:Array<String> = [];
	var songName = PlayState.SONG.song.replace(' ', '-');
	var file:String = Paths.txt(songName + '/dialogue$prefix');
	if (OpenFlAssets.exists(file))
	{
		text = CoolUtil.coolTextFile(file);
		set('dialogue', text);
		set('dialogueLength', text.length);
		return text[0];
	}
	else
	{
		set('dialogue', []);
		set('dialogueLength', 0);
		return null;
	}
});
PlayState.instance.callOnLuas('getDialogueFromText', ['']);

Lua_helper.add_callback(lua, "parseDialogueLine", function(line:String)
{ // also cool asf
	return line.split(':');
});

Lua_helper.add_callback(lua, "setLuaValue", function(key:String, value:String)
{
	set(key, value);
});
...
{
	#if MODS_ALLOWED
	if (FileSystem.exists(Paths.mods(Paths.currentModDirectory + '/' + key)) || FileSystem.exists(Paths.mods(key)))
	{
		return true;
	}
	#end
...
// SHADER SHIT
if (ClientPrefs.effects)
{
	Lua_helper.add_callback(lua, "addChromaticAbberationEffect", function(camera:String, chromeOffset:Float = 5)
	{
		PlayState.instance.addShaderToCamera(camera, new ChromaticAberrationEffect(chromeOffset * 0.001));
	});
...
{ // we should still add the effects, but the callbacks are blank
	Lua_helper.add_callback(lua, "addChromaticAbberationEffect", function(camera:String, chromeOffset:Float = 5)
	{
	});
	Lua_helper.add_callback(lua, "addScanlineEffect", function(camera:String, lockAlpha:Bool = false)
...
Lua_helper.add_callback(lua, "callGlobally", function(func:String, params:String = '')
{
	return call(func, params.split("/"));
});
Lua_helper.add_callback(lua, "alert", function(title:String, message:String)
{
	lime.app.Application.current.window.alert(message, title);
});
Lua_helper.add_callback(lua, "mouseHits", function(obj:String)
{
